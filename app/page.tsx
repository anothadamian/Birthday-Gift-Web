...
export default function Home() {
  ...
  if (!selectedTemplate) return <main className="catalog"><nav><a className="brand" href="#top">made for you <span>✦</span></a><p>tiny websites, big feelings</p></nav><header id="top"><p className="catalog-kicker">digital gifts for your favorite person</p><h1>Choose a feeling.<br /><em>Make it theirs.</em></h1><p className="catalog-intro">One link. Their name, your memories, and a tiny corner of the internet made only for them.</p></header><section className="template-grid" aria-label="Birthday website templates">{templateOptions.map((template) => <button className={`template-card ${template.id}`} key={template.id} onClick={() => setSelectedTemplate(template.id)}><span className="template-number">{template.number}</span><div className="template-art" aria-hidden="true"><i /><b>{template.id === 'vintage' ? 'for you, always' : template.id === 'jar' ? '7 reasons why' : 'best day ever'}</b></div><div className="template-meta"><span><strong>{template.name}</strong><small>{template.tag}</small></span><b>view template ↗</b></div></button>)}</section><footer className="catalog-footer"><span>made with care, sent with love</span><span>three templates · endless stories</span></footer></main>;
